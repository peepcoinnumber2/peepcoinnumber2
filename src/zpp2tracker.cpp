...
CzPP2Tracker::CzPP2Tracker(std::string strWalletFile)
{
...
CzPP2Tracker::~CzPP2Tracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzPP2Tracker::Init()
{
...
bool CzPP2Tracker::Archive(CMintMeta& meta)
{
...
bool CzPP2Tracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzPP2Tracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzPP2Tracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzPP2Tracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzPP2Tracker::GetSerialHashes()
{
...
CAmount CzPP2Tracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzPP2Tracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzPP2Tracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzPP2Tracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzPP2Tracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzPP2Tracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzPP2Tracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzPP2Tracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzPP2Tracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzPP2Tracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzPP2Tracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzPP2Tracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzPP2Tracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzPP2Tracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzPP2Tracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzPP2Tracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzPP2Tracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzpp2 from DB\n", __func__, listDeterministicDB.size());
    }
