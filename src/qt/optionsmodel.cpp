...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizePeepCoinAmount"))
        settings.setValue("nAnonymizePeepCoinAmount", 1000);

    nAnonymizePeepCoinAmount = settings.value("nAnonymizePeepCoinAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizePeepCoinAmount"))
        SoftSetArg("-anonymizepeepcoinamount", settings.value("nAnonymizePeepCoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in peepcoin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizePeepCoinAmount:
            return QVariant(nAnonymizePeepCoinAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizePeepCoinAmount:
            nAnonymizePeepCoinAmount = value.toInt();
            settings.setValue("nAnonymizePeepCoinAmount", nAnonymizePeepCoinAmount);
            emit anonymizePeepCoinAmountChanged(nAnonymizePeepCoinAmount);
            break;
        case CoinControlFeatures:
