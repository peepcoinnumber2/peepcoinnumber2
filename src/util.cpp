...
int nAnonymizePeepCoinAmount = 1000;
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "peepcoin" is a composite category enabling all PeepCoin-related debug output
            if (ptrCategory->count(string("peepcoin"))) {
    ...
    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty peepcoin.conf if it does not exist
