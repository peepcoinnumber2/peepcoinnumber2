...
// The zPP2 block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zPP2 is a hash of the serial
...
{
    //Create an output returning the zPP2 that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZPP2OutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zPP2 output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zPP2", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZPP2OutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zPP2 output", __func__);
...
{
    CzPP2Tracker* zpp2Tracker = pwallet->zpp2Tracker.get();
    CMintMeta meta;
    if (!zpp2Tracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zpp2Tracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a PP2 stake is the outpoint
