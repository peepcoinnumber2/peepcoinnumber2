...
CzPP2Wallet::CzPP2Wallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zpp2 seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZPP2Seed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZPP2Seed_deprecated()) {
                    LogPrintf("%s: Updated zPP2 seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zpp2 seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zpp2 wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzPP2Wallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZPP2Count(nCountLastUsed);
    else if (!walletdb.ReadZPP2Count(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzPP2Wallet::Lock()
{
    seedMaster = 0;
}

void CzPP2Wallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzPP2Wallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZPP2(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzPP2Wallet::LoadMintPoolFromDB()
{
...
void CzPP2Wallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzPP2Wallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzPP2Wallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zpp2Tracker->HasPubcoinHash(pMint.first)) {
...
bool CzPP2Wallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZPP2(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zpp2Tracker which also adds to database
    pwalletMain->zpp2Tracker->Add(dMint, true);
...
void CzPP2Wallet::SeedToZPP2(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzPP2Wallet::GetZerocoinSeed(uint32_t n)
{
...
void CzPP2Wallet::GenerateDeterministicZPP2(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzPP2Wallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzPP2Wallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
