...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.zpp2Tracker = unique_ptr<CzPP2Tracker>(new CzPP2Tracker(cWallet.strWalletFile));
    ...
    BOOST_CHECK_MESSAGE(receipt.GetStatus() == ZPP2_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == ZPP2_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
