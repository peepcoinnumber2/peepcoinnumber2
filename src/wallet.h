...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 PP2 at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zPP2 send
enum ZerocoinSpendStatus {
    ZPP2_SPEND_OKAY = 0,                            // No error
    ZPP2_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZPP2_WALLET_LOCKED = 2,                         // Wallet was locked
    ZPP2_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZPP2_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZPP2_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZPP2_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZPP2_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZPP2_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZPP2_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZPP2_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZPP2_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZPP2_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZPP2_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZPP2_SPENT_USED_ZPP2 = 14,                      // Coin has already been spend
    ZPP2_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZPP2_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZPP2OutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzpp2Auto) const;
    ...
    */
    ...
    CzPP2Wallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzPP2Tracker> zpp2Tracker;
    ...
    void setZWallet(CzPP2Wallet* zwallet)
    {
        zwalletMain = zwallet;
        zpp2Tracker = std::unique_ptr<CzPP2Tracker>(new CzPP2Tracker(strWalletFile));
    }

    CzPP2Wallet* getZWallet() { return zwalletMain; }
    ...
    /** zPP2 reset */
    boost::signals2::signal<void()> NotifyzPP2Reset;
