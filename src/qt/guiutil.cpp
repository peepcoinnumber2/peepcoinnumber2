...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a PeepCoin address (e.g. %1)").arg("P7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no PeepCoin: URI
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient* out)
{
    ...
    //
    //    Cannot handle this later, because peepcoin:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open peepcoin.conf with the associated application */
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a peepcoin.desktop file to the autostart directory:
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the peepcoin app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add peepcoin app to startup item list
